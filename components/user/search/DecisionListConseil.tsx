...
import DecisionConseilCard from "../shared/DecisionConseilCard";
import { set } from "date-fns";

const DecisionListConseil = ({
    initiaDecisions,
    query,
}: {
        initiaDecisions: ConseilDecision[];
    query: {
        ...
        <>
            <div className="grid mt-8 gap-8 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
                {decisions?.map((decision) => {
                    return <DecisionConseilCard key={decision._id} decision={decision} />;
                })}
            </div>
            {hasMore && (
                <div
...
export default DecisionListConseil;
